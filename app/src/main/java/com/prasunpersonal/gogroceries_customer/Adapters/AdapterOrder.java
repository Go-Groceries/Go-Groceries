...
import com.prasunpersonal.gogroceries_customer.databinding.RowOrderBinding;
        ...
        FirebaseFirestore.getInstance().collection("Shops").document(order.getShopId()).get().addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                if (task.getResult() != null && task.getResult().exists()) {
                    ModelShop shop = task.getResult().toObject(ModelShop.class);
                    assert shop != null;
                    Glide.with(context).load(shop.getDp()).placeholder(R.drawable.ic_store).into(holder.binding.productImg);
                    holder.binding.orderTitle.setText(String.format(Locale.getDefault(), "%d product(s) ordered from %s", order.getCartItems().size(), shop.getShopName()));
                }
            } else {
